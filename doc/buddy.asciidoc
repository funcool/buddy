...
- Modular Authentication library.
- Modular Authorization library.
- Regex/decorator based composable access rules system.
- Signing library.
- Password hashing library.
- Minor utils for bytes, string, hex, base64 conversions.
...
signing] library but extended with various signing algorithms.
...
[NOTE]
====
sha384 version is available for each algorithm internally, but is not exposed on public api.

Only HMAC based algorithm supports plain text secret keys, If you want use
more Digital Signature instead of hmac, you must have a key pair (public and private).
...
| Pbkdf2 (with sha256) | `buddy.hashers.pbkdf2` | Recommended
...
hash, hmac, sign and coding that are used by other parts of _buddy_.
...
available in `buddy.core.hmac` namespace.
...
Yes and No. I don't like call "security" library because securty as concept represents a very generic
