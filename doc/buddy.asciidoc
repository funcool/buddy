...
This authorization system encapsulates your hadlers/controllers in one try/catch block
...
it, all builtin authentication backends implements authorization protocol.
...
builtin backends implements specific bahavior.

NOTE: You can overwrite the default handler, passing your own exception handler function as
