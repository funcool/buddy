...
Authorization backend should implement a deterministic workflow for handle not authorized
exceptions. By default, each builtin backend has it own default flow for handle not authorized
requests, but it can to be overwrited with `:unauthorized-handler` parameter on the constructor.
