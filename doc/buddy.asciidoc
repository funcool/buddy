...
For the generic authorization system, _buddy_ exposes protocol for it for easy extend or crete own
backends. Also, previously explained authentication backends, implements authorization protocol, allowing
use one backend for both purposes: authentication or/and authorization.

Each backend has it own default flow for handle not authorized requests, but it can to be overwrited with 
`:unauthorized-handler` parameter to backend constructor.
