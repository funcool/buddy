...
that should return `WWW-Authenticate` header when request is unauthorized) and by default, all
backends comes with default implementation. You can overwrite the default behavior, passing your
own exception handler function as `:unauthorized-handler` keyword parameter to backend constructor.

Below, a complete example of how to setup a basic/generic authorization system
on your ring compatible web application:
...
Access rules system is still flexible and adapt it for a lot of case uses is very
simple.

The simplest way to use access rules, is using `wrap-access-rules` middleware
passing to it an ordered vector of rules. Important: rules are evaluated in order,
...
buddy, behind the scene uses internal library exposed in `buddy.core` namespace that exposes
...
This is a basic pattern followed by buddy:
...
This is a list of available hash algorithms:
...
Hash functions are implemented using clojure protocols. In previous example we have
seen how make hash of a string, but the default implementation is also available 
for: *File*, *URL*, *URI* and *InputStream*.
...
You can extend it for your own types using *buddy.core.hash/Digest* protocol that
looks like this:
...
buddy comes with two varians of *hmac*: standard and salted. Bot are available on 
`buddy.core.hmac` namespace. Like previously explained hash functions, them follows
the `make-<algo>` signature.
...
NOTE: The key parameter can be any type that implements now undocumented
*buddy.core.keys/ISecretKey* protocol (buddy comes with default implementations
for String and byte[])
...
Like hash functions, hmac is also implemented using clojure protocols and exposes
implementation for: String, byte[], *File*, *URL*, *URI* and *InputStream*.
